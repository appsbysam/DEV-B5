...
(function(){
  function rentalByRef(ref){
    const key=String(ref||'').replace(/^#/,'').trim();
    return (state.rentals||[]).find(r=>String(r.uuid)===key||String(r.id)===key)||null;
  }
  ...
  function openVehicle(id){
    if(!id||(state.vehicles||[]).every(v=>String(v.id)!==String(id)))return false;
    if(typeof openVehicleDetails==='function')openVehicleDetails(id);
    return true;
  }
  function consumeDeepLink(){
    const hash=(location.hash||'').replace(/^#/,'');if(!hash)return true;
    let handled=false;
    if(hash.startsWith('rental='))handled=openRental(decodeURIComponent(hash.slice(7)));
    else if(hash.startsWith('rental-number='))handled=openRental(decodeURIComponent(hash.slice(14)));
  ...
  function rentalFromOverdueRow(row){
    ...
    if(direct){const r=rentalByRef(direct);return r?.uuid||r?.id||direct;}
    const m=(row?.textContent||'').match(/#\s*(\d+)/);if(!m)return '';
    const r=rentalByRef(m[1]);return r?.uuid||r?.id||'';
  }
