...
- Added a combined individual-vehicle report showing rental income, vehicle expenses, net result and the underlying transaction history for a selected vehicle.
- Added an overall combined business report showing recorded income, expenses and net result.
- Consolidated the three expected/future-income requirements into one report with a **Period** selector: **Next 7 days**, **Next calendar month**, or **Custom date range**.
- Future expected income is calculated from open/confirmed rental segments overlapping the selected period using their agreed daily rates.
- Marked all remaining Reports Action List requirements complete.
...
- Reworked vehicle-card child actions so Purchase Details, Add Expense and Add Maintenance remain inside the open vehicle context.
